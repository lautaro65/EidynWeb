...
export async function createCheckoutSessionAction(locale: string) {
  try {
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      ...
      return { url: `/${locale}/dashboard/shop/billing?mock_success=true` };
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/${locale}/dashboard/shop/billing?success=true`,
      cancel_url: `${baseUrl}/${locale}/dashboard/shop/billing?canceled=true`,
      metadata: {
        tenantId: tenant.id,
      },
    });
...
export async function createCustomerPortalAction(locale: string) {
  try {
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      ...
      return { url: `/${locale}/dashboard/shop/billing?mock_portal=true` };
    }
    ...
    const portalSession = await stripe.billingPortal.sessions.create({
      ...
      return_url: `${baseUrl}/${locale}/dashboard/shop/billing`,
    });
